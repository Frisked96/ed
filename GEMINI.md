...
- `view.py`: Handles high-performance map rendering using chunk-based caching, procedural glyph generation, and smart animation redraws.
...
- `map_io.py`: Handles file system operations for loading/saving maps, configurations, and macros, including the `tiles/` directory structure.
- `tiles.py`: Manages the `REGISTRY` of tile definitions (including `TileAnimation` models), their characters, colors, and metadata.
...
- `tiles/`: Directory containing tile definitions (`custom_tiles.json` for user data, `animated_tiles.json` for defaults).
